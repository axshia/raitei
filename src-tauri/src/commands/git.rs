...
use crate::git::{
    self, ConflictFileContent, ConflictResolution, ConflictState, GitStatus, WorktreeInfo,
};
    ...
    blocking(move || {
        ...
        for w in v.iter_mut() {
            w.task_id = tasks
                .iter()
                .find(|t| {
                    if t.worktree_path == w.path {
                        return true;
                    }
                    // macOS may report /private/var while a saved task uses /var.
                    match (
                        std::fs::canonicalize(&t.worktree_path),
                        std::fs::canonicalize(&w.path),
                    ) {
                        (Ok(task_path), Ok(git_path)) => task_path == git_path,
                        _ => false,
                    }
                })
                .map(|t| t.id.clone());
...
#[tauri::command]
pub async fn complete_base_merge(
    state: State<'_, AppState>,
    task_id: String,
    push: bool,
) -> AppResult<ConflictState> {
...
#[tauri::command]
pub async fn request_agent_conflict_resolution(
    state: State<'_, AppState>,
    task_id: String,
) -> AppResult<AgentRunInfo> {
    let s = state.inner().clone();
    blocking(move || {
        let t = s.store.get_task(&task_id)?;
